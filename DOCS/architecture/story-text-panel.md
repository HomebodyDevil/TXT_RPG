...
크기 변경 콜백은 Canvas를 즉시 강제 갱신하지 않습니다. 갱신 필요 상태만 기록하고, 필수 참조가 준비된 다음 `LateUpdate`에서 같은 프레임의 요청을 병합하여 한 번 처리합니다. 이 방식은 부모 `FlexibleContentLayoutGroup`의 `SetChildAlongAxis` 실행 중 Canvas 레이아웃으로 재진입하는 것을 방지합니다.

활성화 시 참조가 아직 연결되지 않았다면 누락 필드를 한 번만 경고하고 갱신을 보류합니다. 이후 참조가 연결되면 스크롤 이벤트를 등록하고 옵션·레이아웃·최신 메시지 위치 갱신을 자동으로 재개합니다. 코루틴이 한 프레임 대기한 뒤에도 활성 상태와 필수 참조를 다시 검사합니다. 비활성화와 파괴 시에는 예약 갱신, Reveal·스크롤 코루틴과 실제 구독 대상의 이벤트를 정리하므로 참조가 교체되더라도 이전 ScrollRect에 구독이 남지 않습니다.
