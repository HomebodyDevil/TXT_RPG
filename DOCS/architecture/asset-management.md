...
`IAssetProvider.LoadAsync<T>`는 `AssetLease<T>`를 반환합니다. Lease를 해제하면 대응하는 Addressables Handle도 정확히 한 번 해제됩니다. `AssetScope`는 화면이나 목록에서 로드한 Lease를 보관하고 Dispose 시 역순으로 모두 해제합니다.
